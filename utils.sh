...
patch_apk() {
	...
	echo "java -jar ${RV_CLI_JAR} -a ${stock_input} -o ${patched_output} -b ${RV_PATCHES_JAR} --keystore=ks.keystore ${patcher_args}"
        java -jar --rip-lib x86 --rip-lib x86_64 "$RV_CLI_JAR" -a "$stock_input" -o "$patched_output" -b "$RV_PATCHES_JAR" --keystore=ks.keystore $patcher_args
}
